...
| # | מה לעשות | איפה |
|---|----------|------|
| 1 | Supabase – פרויקט + הרצת `schema.sql` + העתקת URL ו־anon key | [חלק 1](#חלק-1-supabase-מסד-נתונים--התחברות) |
| 2 | GitHub – repo + דחיפת כל הפרויקט (כולל `netlify-app`) | [חלק 2](#חלק-2-github-העלאת-הקוד) |
| 3 | Netlify – חיבור ל־repo + **Base directory:** `netlify-app` + **Publish:** `dist` | [חלק 3.1–3.2](#שלב-32--הגדרות-בנייה-איפה-netlify-יבנה-את-האתר) |
| 4 | Netlify – הוספת משתנים: `PUBLIC_SUPABASE_URL` ו־`PUBLIC_SUPABASE_ANON_KEY` | [חלק 3.3](#שלב-33--הוספת-משתני-הסביבה-מפתחות-supabase) |
| 5 | Netlify – **Deploy site** | [חלק 3.4](#שלב-34--הפעלת-הדיפלוי) |
| 6 | Supabase – **Site URL** ו־**Redirect URLs** = כתובת האתר ב־Netlify | [חלק 4](#חלק-4-supabase--הגדרת-כתובת-האתר-להשלמת-ההתחברות) |
| 7 | בדיקה – הרשמה, התחברות, יצירת עמוד ופוסט | [חלק 5](#חלק-5-בדיקה) |

אם הבנייה נכשלת – וודא ש־**Base directory** = `netlify-app` ושהתיקייה `netlify-app` קיימת ב־GitHub.

**אם `netlify-app` כבר ב־GitHub – מה הלאה:**  
1. Netlify → **Base directory:** `netlify-app`, **Publish:** `dist`, **Build command:** `npm run build`.  
2. Netlify → **Environment variables:** `PUBLIC_SUPABASE_URL` ו־`PUBLIC_SUPABASE_ANON_KEY`.  
3. **Deploy site**.  
4. Supabase → **Authentication** → **URL Configuration** → Site URL ו־Redirect URLs = כתובת האתר ב־Netlify.  
5. בדיקה: הרשמה, התחברות, יצירת עמוד ופוסט.
